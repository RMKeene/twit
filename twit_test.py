...
class TestTwit(unittest.TestCase):
    ...
    def test_EA_compute_twit_single_dimension(self):
        # 1 to 1
        t = compute_twit_single_dimension(1, 1, 5, 5, 10.0, 10.0)
    ...
    def test_FA_compute_twit_multi_dimension(self):
        t = compute_twit_multi_dimension(2, np.array([0, 3, 0, 4, 0, 4, 0, 1], dtype=np.int64), np.array([0.0, 1.0, 1.0, 1.0], dtype=np.double))
        t = unpack_twit_multi_axis(t)
        ...
        ttt = t
        tt = pack_twit_multi_axis(ttt)
        t = unpack_twit_multi_axis(tt)
        self.assertEqual(2, t[0])
        self.assertEqual(8, t[1][0])
        nt.assert_array_almost_equal(t[1][1], [0, 0, 1, 1, 2, 2, 3, 3])
        nt.assert_array_almost_equal(t[1][2], [0, 1, 1, 2, 2, 3, 3, 4])
    ...
    def test_HA_apply_twit(self):
        ...
        make_and_apply_twit(1, np.array([0, 4, 0, 4], dtype=np.int64),  np.array([1.0, 1.0], dtype=np.float64), t1, t2, 1)
        a = [1., 1., 1., 1., 1.]
        ...
        t1 = np.ones((5))
        t2 = np.ones((4))
        make_and_apply_twit(1, np.array([0, 4, 0, 3], dtype=np.int64),  np.array([0.0, 1.0], dtype=np.float64), t1, t2, 1)
        ...
        t1[1, 1] = 0.5
        t2 = np.ones((4, 5))
        make_and_apply_twit(2, np.array([0, 4, 0, 3, 0, 2, 0, 4], dtype=np.int64),  np.array([1.0, 1.0, 1.0, 1.0], dtype=np.float64), t1, t2, 1)
        ...
        t1[1, 1] = 0.5
        t2 = np.ones((4, 5))
        make_and_apply_twit(2, np.array([0, 4, 0, 3, 0, 2, 0, 4], dtype=np.int64),  np.array([0.0, 1.0, 1.0, 1.0], dtype=np.float64), t1, t2, 1)
        ...
        print("-----------------------------------------------------")
        make_and_apply_twit(3, np.array([1, 3, 0, 2, 2, 3, 0, 1, 0, 2, 0, 0], dtype=np.int64),  np.array([0.9, 0.5, 1.0, 1.0, 1.0, 1.0], dtype=np.float64), t1, t2, 1)
