...
class TestTwit(unittest.TestCase):
    ...
    def test_AAA_match_shape_lengths(self):
        npt.assert_array_almost_equal(match_shape_lengths((4, 2), (3, 5)), ((4, 2), (3, 5)))
        npt.assert_array_almost_equal(match_shape_lengths((7, 4, 2), (3, 5)), ((7, 4, 2), (1, 3, 5)))
        npt.assert_array_almost_equal(match_shape_lengths((4, 2), (9, 3, 5)), ((1, 4, 2), (9, 3, 5)))
        npt.assert_array_almost_equal(match_shape_lengths((2,), (9, 3, 5)), ((1, 1, 2), (9, 3, 5)))
        pass
    ...
    def test_BB_parse_prims(self):
        ...
        a = try_parse_float(None, 3.14159)
        self.assertEqual(a, (3.14159, False))
        a = try_parse_float("", 3.14159)
        self.assertEqual(a, (3.14159, False))
        a = try_parse_float("w3", 3.14159)
        self.assertEqual(a, (3.14159, False))
        a = try_parse_float("3w", 3.14159)
        self.assertEqual(a, (3.14159, False))
        a = try_parse_float("12.3", 3.14159)
        self.assertEqual(a, (12.3, True))
        a = try_parse_float("-7.2", 3.14159)
        self.assertEqual(a, (-7.2, True))
        a = try_parse_float("-", 3.14159)
        self.assertEqual(a, (3.14159, False))
        a = try_parse_float("-.3", 3.14159)
        self.assertEqual(a, (-0.3, True))

        a = try_parse_int()
        self.assertEqual(a, (0, False))
        a = try_parse_int(None, 3)
        self.assertEqual(a, (3, False))
        a = try_parse_int("", 3)
        self.assertEqual(a, (3, False))
        a = try_parse_int("w4", 3)
        self.assertEqual(a, (3, False))
        a = try_parse_int("4w", 3)
        self.assertEqual(a, (3, False))
        a = try_parse_int("12", 3)
    ...
    def test_AB_range_series(self):
        ...
        a: RangeSeries = find_range_series_multipliers(0, 3, 3, 9, 1)
        nt.assert_almost_equal(a.idxs, t1[0])
        nt.assert_almost_equal(a.values, t1[1])
        a: RangeSeries = find_range_series_multipliers(0, 3, 3, 9, 2)
        nt.assert_almost_equal(a.idxs, t2[0])
        nt.assert_almost_equal(a.values, t2[1])
        a: RangeSeries = find_range_series_multipliers(0, 3, 3, 9, 3)
        nt.assert_almost_equal(a.idxs, t3[0])
        nt.assert_almost_equal(a.values, t3[1])
        a: RangeSeries = find_range_series_multipliers(3, 0, 3, 9, 1)
        nt.assert_almost_equal(a.idxs, t1[0])
        nt.assert_almost_equal(a.values, t1[1])
        a: RangeSeries = find_range_series_multipliers(0, 3, 9, 3, 2)
        nt.assert_almost_equal(a.idxs, t2[0])
        nt.assert_almost_equal(a.values, t2[1])
        a: RangeSeries = find_range_series_multipliers(0, 3, 3, 9, 3)
        nt.assert_almost_equal(a.idxs, t3[0])
        ...
        # One to Many
        a: RangeSeries = find_range_series_multipliers(1, 1, 3, 9, 1)
        ...
        with self.assertRaises(Exception):
            find_range_series_multipliers(0, 3, 3, 9, 4)
        with self.assertRaises(Exception):
            find_range_series_multipliers(None, None, 3, 9, 2)
        with self.assertRaises(Exception):
            find_range_series_multipliers(0, 3, None, None, 2)
        with self.assertRaises(Exception):
            find_range_series_multipliers(0, 3, 3, 9, -1)
        with self.assertRaises(Exception):
            find_range_series_multipliers(-1, 3, 3, 9, 1)
        with self.assertRaises(Exception):
            find_range_series_multipliers(0, 3, 3, -9, 1)
    ...
    def test_EA_compute_twit_single_dimension(self):
        # 1 to 1
        t: TwitSingleAxis = compute_twit_single_dimension(1, 1, 5, 5, 10.0, 10.0)
        ...
        nt.assert_almost_equal(t.weights,
                               ([5., 3.571428571428571, 2.678571428571429, 7.5, 3.7500000000000004, 5.0, 10.0]))
        # 5 to 3
    ...
    def test_FA_compute_twit_multi_dimension(self):
        t: TwitMultiAxis = compute_twit_multi_dimension(2, np.array([0, 3, 0, 4, 0, 4, 0, 1], dtype=np.int64),
                                                        np.array([0.0, 1.0, 1.0, 1.0], dtype=np.double))
    ...
    def test_HA_apply_twit(self):
        ...
        t: TwitMultiAxis = compute_twit_multi_dimension(1, np.array([0, 4, 0, 4], dtype=np.int64),
        ...
        t1 = np.ones(5)
        t2 = np.ones(4)
        t: TwitMultiAxis = compute_twit_multi_dimension(1, np.array([0, 4, 0, 3], dtype=np.int64),
                                                        np.array([1.0, 1.0], dtype=np.float64))
        apply_twit(t, t1, t2, 1)
        a = [1., 1., 1., 1.]
        npt.assert_array_almost_equal(t2, a)
        ...
        t1[1, 1] = 0.5
        t2 = np.ones((4, 5))
        t: TwitMultiAxis = compute_twit_multi_dimension(2, np.array([0, 4, 0, 3, 0, 2, 0, 4], dtype=np.int64),
        ...
        twt = compute_twit_multi_dimension(2, np.array([0, 4, 0, 3, 0, 2, 0, 4], dtype=np.int64),
                                           np.array([0.0, 1.0, 1.0, 1.0], dtype=np.float64))
        apply_twit(twt, t1, t2, 1)
        npt.assert_array_almost_equal(t2, a)

        # And test reuse of twt on different tensor instances.
        t1 = np.ones((5, 3))
        t1[1, 1] = 0.5
        t2 = np.ones((4, 5))
        apply_twit(twt, t1, t2, 1)
        ...
        t2 = np.ones((3, 5, 1))
        # print("-----------------------------------------------------")
        twt: TwitMultiAxis = compute_twit_multi_dimension(3, np.array([1, 3, 0, 2, 2, 3, 0, 1, 0, 2, 0, 0],
                                                                      dtype=np.int64),
                                                          np.array([0.9, 0.5, 1.0, 1.0, 1.0, 1.0], dtype=np.float64))
        apply_twit(twt, t1, t2, 1)
