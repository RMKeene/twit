...
class TestTwitc(unittest.TestCase):
    ...
    def test_EA_compute_twit_single_dimension(self):
        ...
        nt.assert_almost_equal(t, ([1, 1, 2, 2, 2, 3, 3], [0, 1, 1, 2, 3, 3, 4],
        ...
        nt.assert_almost_equal(t, (
        [1, 2, 2, 3, 3, 4], [0, 0, 1, 1, 2, 2], [3.75, 1.66666667, 3.33333333, 4.16666667, 2.08333333, 7.5]))
        # 5 to 2
        t = twitc.compute_twit_single_dimension(0, 4, 0, 1, 1.0, 1.0)
        nt.assert_almost_equal(t, (
        [0, 1, 2, 3, 1, 2, 3, 4], [0, 0, 0, 0, 1, 1, 1, 1], [0.4, 0.3, 0.2, 0.1, 0.1, 0.2, 0.3, 0.4]))
        pass

    def test_FA_compute_twit_multi_dimension(self):
        t = twitc.compute_twit_multi_dimension(2, np.array([0, 3, 0, 4, 0, 4, 0, 1], dtype=np.int64),
    ...
    def test_HA_apply_twit(self):
        ...
        twitc.make_and_apply_twit(1, np.array([0, 4, 0, 4], dtype=np.int64), np.array([1.0, 1.0], dtype=np.float64), t1,
                                  t2, 1)
        a = [1., 1., 1., 1., 1.]
        ...
        t1 = np.ones(5)
        t2 = np.ones(4)
        twitc.make_and_apply_twit(1, np.array([0, 4, 0, 3], dtype=np.int64), np.array([1.0, 1.0], dtype=np.float64), t1,
                                  t2, 1)
        a = [1., 1., 1., 1.]
        npt.assert_array_almost_equal(t2, a)
        ...
        t1 = np.ones(5)
        t2 = np.ones(4)
        twitc.make_and_apply_twit(1, np.array([0, 4, 0, 3], dtype=np.int64), np.array([0.0, 1.0], dtype=np.float64), t1,
                                  t2, 1)
        ...
        t1[1, 1] = 0.5
        t2 = np.ones((4, 5))
        twitc.make_and_apply_twit(2, np.array([0, 4, 0, 3, 0, 2, 0, 4], dtype=np.int64),
                                  np.array([1.0, 1.0, 1.0, 1.0], dtype=np.float64), t1, t2, 1)
        ...
        t2 = np.ones((4, 5))
        twitc.make_and_apply_twit(2, np.array([0, 4, 0, 3, 0, 2, 0, 4], dtype=np.int64),
                                  np.array([0.0, 1.0, 1.0, 1.0], dtype=np.float64), t1, t2, 1)
        ...
        t1[1, 1] = 0.5
        t2 = np.ones((4, 5))
        twt = twitc.compute_twit_multi_dimension(2, np.array([0, 4, 0, 3, 0, 2, 0, 4], dtype=np.int64),
        ...
        # print("-----------------------------------------------------")
        twitc.make_and_apply_twit(3, np.array([1, 3, 0, 2, 2, 3, 0, 1, 0, 2, 0, 0], dtype=np.int64),
                                  np.array([0.9, 0.5, 1.0, 1.0, 1.0, 1.0], dtype=np.float64), t1, t2, 1)
